    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1E293B] rounded-2xl p-5 border border-[#334155]"><h3 className="font-semibold text-white mb-4">{t.costByPosition}</h3>
          <ResponsiveContainer width="100%" height={250}><PieChart><Pie data={costByPos} cx="50%" cy="50%" outerRadius={95} paddingAngle={3} dataKey="value" label={({ name, percent }: { name: string; percent: number }) => `${name} ${(percent * 100).toFixed(0)}%`}>{costByPos.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip formatter={(v: number) => fmt(v)} contentStyle={tipStyle} /></PieChart></ResponsiveContainer></div>
      </div>
    </div>
  );
