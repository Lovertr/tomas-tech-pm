...
export async function GET(req: NextRequest) {
  ...
  const titleMap = { th: "คู่มือผู้ใช้งาน — CONSERTECH PM", en: "User Manual — CONSERTECH PM", jp: "ユーザーマニュアル — CONSERTECH PM" };
...
<style>
  ...
  .cover h1 { font-size: 28pt; color: #0072B8; margin-bottom: 8px; }
  .cover .subtitle { font-size: 14pt; color: #64748b; }
  .cover .company { font-size: 12pt; color: #F59E0B; margin-top: 40px; font-weight: bold; }
  .cover .date { font-size: 10pt; color: #94a3b8; margin-top: 8px; }
  h2 { color: #0072B8; font-size: 16pt; border-bottom: 2px solid #4DB5D6; padding-bottom: 4px; margin-top: 32px; }
  h3 { color: #1e40af; font-size: 13pt; margin-top: 20px; }
  p { margin: 4px 0; }
  .page-break { page-break-before: always; }
  .article { margin-bottom: 20px; }
  .toc { margin: 40px 0; }
  .toc h2 { border-bottom-color: #0072B8; }
  .toc ul { list-style: none; padding: 0; }
  .toc li { padding: 4px 0; border-bottom: 1px dotted #e2e8f0; }
  .toc li span { color: #0072B8; font-weight: 600; }
</style>
</head>
<body>
  <div class="cover">
    ...
    <div class="company">CONSERTECH CO., LTD.</div>
  ...
  return new NextResponse(html, {
    headers: {
      ...
      "Content-Disposition": `inline; filename="CONSERTECH_User_Manual_${lang}.html"`,
    },
  });
}
