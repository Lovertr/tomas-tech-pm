...
const STATUS_META: Record<string, { color: string; bg: string; label: string; icon: typeof CheckCircle2 }> = {
  pending:     { color: "#94A3B8", bg: "bg-slate-500/20 text-slate-300", label: "Pending",     icon: Clock },
  in_progress: { color: "#F7941D", bg: "bg-orange-500/20 text-orange-300", label: "In Progress", icon: Flag },
...
export default function MilestonesPanel({ projects, filterProjectId = "all", canManage = true, refreshKey = 0 }: Props) {
  ...
  const Row = (m: Milestone) => {
    ...
    return (
      <div key={m.id} className="bg-[#1E293B] border border-[#334155] rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-slate-500 mt-1">
            ...
            {m.status !== "completed" && (
              <span className={`ml-2 ${diff < 0 ? "text-red-400" : diff <= 7 ? "text-orange-400" : ""}`}>
        ...
        <div className="flex items-center gap-1">
          ...
          {canManage && <button onClick={() => setEditing(m)} className="p-1.5 text-slate-400 hover:text-white"><Edit3 size={14} /></button>}
          {canManage && <button onClick={() => remove(m.id)} className="p-1.5 text-red-400 hover:text-red-300"><Trash2 size={14} /></button>}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#1E293B] rounded-2xl border border-[#334155] w-full max-w-lg p-6 space-y-4" onClick={e => e.stopPropagation()}>
        <h3 className="text-lg font-semibold text-white">{initial ? "แก้ไข Milestone" : "เพิ่ม Milestone"}</h3>
        <div>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">ชื่อ Milestone *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs text-slate-400 mb-1">วันที่ครบกำหนด *</label>
