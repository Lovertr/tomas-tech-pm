...
export default function IssuesPanel({ projects, members, filterProjectId = "all", canManage = true, refreshKey = 0 }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex rounded-xl overflow-hidden border border-gray-300 w-fit">
        {(["active", "resolved", "all"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 text-xs font-medium ${filter === f ? "text-gray-900" : "text-gray-600"}`}
            style={filter === f ? { background: "#E0E7FF" } : { background: "#F3F4F6" }}>
      ...
      <div className="space-y-2">
        {filtered.map(i => {
          ...
          return (
            <div key={i.id} className="bg-white border border-gray-300 rounded-xl p-3 md:p-4 flex items-start gap-2 md:gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-200/50 text-gray-700">{i.projects?.project_code || "—"}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded text-gray-900" style={{ background: SEV_COLOR[i.severity] }}>{SEV_LBL[i.severity]}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded text-gray-900" style={{ background: STATUS_COLOR[i.status] }}>{STATUS_LBL[i.status]}</span>
                </div>
                <div className={`text-sm font-medium ${isResolved ? "text-gray-600 line-through" : "text-gray-900"}`}>{i.title}</div>
                {i.description && (
                  <>
                    <div className="text-xs text-gray-600 mt-0.5">{i.description}</div>
              ...
              {canManage && (
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => setEditing(i)} className="p-1.5 text-gray-600 hover:text-gray-900"><Edit3 size={14} /></button>
                  <button onClick={() => remove(i.id)} className="p-1.5 text-red-600 hover:text-red-700"><Trash2 size={14} /></button>
                </div>
              )}
            </div>
          );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center md:p-4 bg-black/10 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#FFFFFF] rounded-t-2xl md:rounded-2xl border-t md:border border-[#E5E7EB] w-full max-w-lg p-4 md:p-6 space-y-4 max-h-[95vh] md:max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="text-base md:text-lg font-semibold text-gray-900">{initial ? "แก้ไข Issue" : "เพิ่ม Issue"}</h3>
        <Field label="โครงการ *">
        ...
        <Field label="รายละเอียด">
          <textarea rows={3} className="w-full bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm"
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button onClick={submit} disabled={saving} className="px-4 py-2 bg-[#1E40AF] hover:bg-[#2563EB] text-gray-900 rounded-lg text-sm disabled:opacity-50">
