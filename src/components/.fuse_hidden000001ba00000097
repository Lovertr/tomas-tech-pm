...
export default function PermissionsModal({ user, open, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white border border-gray-300 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
        {/* header */}
        <div className="flex items-center justify-between border-b border-gray-300 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">{user.display_name} <span className="text-gray-500">({user.username})</span></div>
            </div>
          </div>
        ...
        {/* toolbar */}
        <div className="px-5 py-3 border-b border-gray-300 flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
            <input value={filter} onChange={e => setFilter(e.target.value)} placeholder="ค้นหาเมนู..."
              className="w-full bg-gray-50 border border-gray-300 rounded-lg pl-9 pr-3 py-1.5 text-sm text-gray-900" />
          </div>
          <div className="flex items-center gap-1 text-xs">
            <span className="text-gray-500 mr-1">ตั้งทั้งหมด:</span>
            {LEVELS.map(l => (
          ...
          <button onClick={reset} disabled={saving}
            className="px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 border border-gray-300 rounded-lg flex items-center gap-1 disabled:opacity-50">
            <RotateCcw size={12} /> ใช้ค่า Default
          </button>
        </div>

        {/* legend */}
        <div className="px-5 py-2 border-b border-gray-300 flex items-center gap-3 flex-wrap text-[10px]">
          <span className="text-gray-500">ระดับสิทธิ์:</span>
          {LEVELS.map(l => (
            <span key={l} className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full" style={{ background: PERM_COLOR[l] }} />
              <span className="text-gray-600">{l} = {PERM_LABEL_TH[l]}</span>
            </span>
          ))}
          <span className="text-gray-500 ml-auto">● = override (ค่า default ของ role ถูกแทนที่)</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {!loading && grouped.map(g => (
            <div key={g.cat} className="bg-gray-50 border border-gray-300 rounded-xl">
              <div className="flex items-center justify-between px-4 py-2 border-b border-gray-300">
                <h4 className="text-sm font-semibold text-orange-400">{CATEGORY_LABEL[g.cat] || g.cat}</h4>
                <div className="flex items-center gap-1">
                  <span className="text-[10px] text-gray-500 mr-1">ตั้งหมวดนี้:</span>
                  {LEVELS.map(l => (
              ...
              <div className="divide-y divide-[#FFFFFF]">
                {g.items.map(m => {
                  ...
                  return (
                    <div key={m.key} className="flex items-center gap-3 px-4 py-2 hover:bg-white/50">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-gray-500 font-mono">{m.key}</div>
                      </div>
                      <div className="flex items-center gap-1">
                        {LEVELS.map(l => (
