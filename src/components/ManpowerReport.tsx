    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={exportXLSX} disabled={logs.length === 0}
          className="flex items-center gap-1.5 px-3 py-2 bg-[#F7941D] hover:bg-[#FFA630] disabled:opacity-50 text-white text-sm font-medium rounded-lg">
          <Download size={16} /> Export Excel
        </button>
      </div>

      <div className="flex flex-wrap gap-3 bg-[#1E293B] border border-[#334155] rounded-lg p-3 items-end">
        <div>
          <label className="block text-xs text-slate-400 mb-1"><Calendar size={12} className="inline mr-1" />Start</label>
          <input type="date" className="bg-[#0F172A] border border-[#334155] rounded px-2 py-1.5 text-sm text-white" value={start} onChange={e => setStart(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1">End</label>
          <input type="date" className="bg-[#0F172A] border border-[#334155] rounded px-2 py-1.5 text-sm text-white" value={end} onChange={e => setEnd(e.target.value)} />
        </div>
        <div>
        ...
        <div className="text-xs text-slate-400 ml-2">{loading ? "Loading..." : `${logs.length} entries`}</div>
      </div>
            ...
            {byProject.map((p, i) => {
              ...
              return (
                <tr key={i} className="border-t border-[#334155]">
                  <td className="px-4 py-2 text-white">{projName(p.project)}</td>
                  <td className="px-4 py-2 text-right text-slate-300">{p.hours.toFixed(1)}</td>
                  <td className="px-4 py-2 text-right text-[#F7941D] font-medium">฿{Math.round(p.cost).toLocaleString()}</td>
                  <td className="px-4 py-2 text-right text-slate-300">฿{Math.round(p.billable).toLocaleString()}</td>
                  <td className="px-4 py-2 text-right text-slate-400">{budget > 0 ? `฿${Math.round(budget).toLocaleString()}` : "—"}</td>
                  <td className={`px-4 py-2 text-right font-medium ${over ? "text-red-400" : "text-slate-300"}`}>
      ...
      <Section title="By Member" count={byMember.length}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {byMember.map((m, i) => (
              <tr key={i} className="border-t border-[#334155]">
                <td className="px-4 py-2 text-white">{memName(m.member)}</td>
                <td className="px-4 py-2 text-slate-300" style={{ color: m.position?.color ?? undefined }}>{posName(m.position)}</td>
                <td className="px-4 py-2 text-right text-slate-300">{m.hours.toFixed(1)}</td>
                <td className="px-4 py-2 text-right text-[#F7941D] font-medium">฿{Math.round(m.cost).toLocaleString()}</td>
                <td className="px-4 py-2 text-right text-slate-300">฿{m.hours > 0 ? Math.round(m.cost / m.hours).toLocaleString() : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Section title="By Position" count={byPosition.length}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {byPosition.map((p, i) => (
              <tr key={i} className="border-t border-[#334155]">
                <td className="px-4 py-2 text-white" style={{ color: p.position?.color ?? undefined }}>{posName(p.position)}</td>
                <td className="px-4 py-2 text-right text-slate-300">{p.memberCount.size}</td>
                <td className="px-4 py-2 text-right text-slate-300">{p.hours.toFixed(1)}</td>
                <td className="px-4 py-2 text-right text-[#F7941D] font-medium">฿{Math.round(p.cost).toLocaleString()}</td>
              </tr>
            ))}
  ...
  return (
    <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-[#334155] flex items-center justify-between">
        ...
        <div className="text-xs text-slate-400">{count} rows</div>
      </div>
